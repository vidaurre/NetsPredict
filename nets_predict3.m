...
function [predictedY,stats,predictedYC,YoutC,predictedYmean,beta,Cin,grotperms,folds] ...
    = nets_predict3(Yin,Xin,family,parameters,varargin)
...
for perm=1:Nperm
    ...
    for ifold = 1:length(folds)
        ...
        if enet && relaxed
            if strcmp(family{2},'multinomial'), QpredictedYp = Inf(QN,q,nlambda);
            else QpredictedYp = Inf(QN,nlambda);
            end
            for Qifold = 1:length(Qfolds)
                QJ = Qfolds{Qifold};
                Qji=setdiff(1:QN,QJ); QX=QXin(Qji,:);  QY=QYin(Qji,:);
                if strcmp(family{2},'gaussian'), Qmy=mean(QY);  QY=QY-Qmy; end
                switch Method
                    case 'glmnet'
                        glmfit = nets_glmnet(QX,QY,family{2},0,tmpnm,options);
                    case 'lasso'
                        glmfit = struct();
                        [glmfit.beta,lassostats] = lasso(QX,QY,'Alpha',options.alpha,'NumLambda',nlambda);
                        glmfit.lambda = lassostats.Lambda;
                end
                if Qifold == 1 && enet
            ...
            % Pick the one with the lowest deviance (=quadratic error for family{1}="gaussian")
            if strcmp(family{2},'gaussian') % it's actually QN*log(sum.... but it doesn't matter
                Qdev = sum(( QpredictedYp(:,1:length(glmfit.lambda)) - ...
                    repmat(QYinCOMPARE,1,length(glmfit.lambda))).^2) / QN; Qdev = Qdev';
            elseif strcmp(family{2},'multinomial')
                Qdev = Inf(length(glmfit.lambda),1);
                for i=1:length(glmfit.lambda), Qdev(i) = ...
                        - sum(log(sum(QYinCOMPARE .* QpredictedYp(:,:,i) ,2))); end
            elseif strcmp(family{2},'poisson')
                Ye = repmat(QYinCOMPARE,1,length(glmfit.lambda));
                Qdev = sum(Ye .* log( (Ye+(Ye==0)) ./ QpredictedYp(:,1:length(glmfit.lambda))) - ...
                    (Ye - QpredictedYp(:,1:length(glmfit.lambda)))); Qdev = Qdev';
            else % cox
                failures = find(QYinCOMPARE(:,2) == 1)';
                Qdev = zeros(length(glmfit.lambda),1);
                for i=1:length(glmfit.lambda)
                    for n=failures, Qdev(i) = Qdev(i) + log( QpredictedYp(n,i) / ...
                            sum(QpredictedYp(QYinCOMPARE(:,1) >= QYinCOMPARE(n,1),i)) ); end
                end; Qdev = -2 * Qdev;
            end
            ...
            glmfit = nets_glmnet(X,Y,family{2},0,tmpnm,options);
            %if strcmp(family{1},'multinomial'),
            %    fprintf('It %d-2, %d variables, alpha=%f,lambda=%f \n',ifold,sum(glmfit.beta{1}(:,end)~=0 ) , ...
            %        options.alpha,options.lambda(end))
            %else fprintf('It %d-2, %d variables, alpha=%f,lambda=%f \n',ifold,sum(glmfit.beta(:,end)~=0 ), ...
            %        options.alpha,options.lambda(end)); end
            if perm==1
                if strcmp(family{2},'multinomial')
                    beta(groti,ifold,2) = glmfit.beta{1}(:,end);
                else
                    beta(groti,ifold,2) = glmfit.beta(:,end);
                end
            end
        end
        ...
        if biascorrect % we do this in the original space
            Yhattrain = Xin(ji,groti) * glmfit.beta(:,end) + repmat(my,length(ji),1);
            if deconfounding(2)

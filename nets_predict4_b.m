...
function [predictedY,stats,predictedYC,YoutC,predictedYmean,predictedY_nonrelaxed,beta,Cin,grotperms,folds] ...
    = nets_predict4_b(Yin,Xin,family,parameters,varargin)
...
if ~isfield(parameters,'deconfounding'), deconfounding=[1 1];
...
if ~isfield(parameters,'relaxed'), relaxed = strcmp(family,'gaussian');
...
for perm=1:Nperm
    ...
    if relaxed 
        predictedYp_nonrelaxed = zeros(N,1);
    end
    ...
    for ifold = 1:length(folds) 
        ...
        if relaxed
            XJ=Xin(J,groti0);
            predictedYp_nonrelaxed(J) = XJ * glmfit.beta(:,end) + repmat(my,length(J),1);
        end
        ...
        if deconfounding(2) % in order to later estimate prediction accuracy in deconfounded space
            ...
            if ~isempty(betaY)
                ...
                predictedYp_nonrelaxed(J,:) = nets_confound(predictedYp_nonrelaxed(J,:),confounds(J,:),family{2},betaY,interceptY);
    ...
    if perm==1
        ...
        predictedY_nonrelaxed = predictedYp_nonrelaxed;
